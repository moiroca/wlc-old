                ...
                <div class="panel panel-default">
                  ...
                  <div class="panel-body">
                    <?php if($_POST['submit']){
                     ...
                     echo $origqty ."-";
                     echo $iquantity ." ";
                     if($iquantity > $origqty || $origqty==0){
                      
                          echo"
                          <script>
                            alert('Failed to add your request, Total number of borrowed equipment is equal to zero or Number of For Replace Equipment is more than the total number of Equipment borrowed');
                            
                          </script>
                          ";
                        }
                      if($istatus == "For Replace"){
                        if($iquantity <= $origqty){
                          $totalquantity = $origqty-$iquantity;

                        $query = "INSERT INTO requisition(area_id, item_id,item_area,item_quantity,item_status,requisition_date,requisition_status) 
                              VALUES ('".$areaid."','".$iid."','".$iarea."','".$iquantity."','".$istatus."', '".date("y-m-d")."', 'Pending')";
                        $result = mysql_query($query) or die ("Error in query:" .mysql_error());

                        $query3 = mysql_query("Update items SET item_quantity='".$totalquantity."' where area_id='".$area_id."' and item_id='".$iid."'");
                            echo $totalquantity;
                            echo"
                            <meta http-equiv='refresh' content='0;url= inventory_add_replace.php?area_id=".$_SESSION['area_id']."'>
                            <script>
